...
// const y = Array.from({length: 8}, () => 3);
// console.log(y);

// const z = Array.from({length: 7}, (_, i) => i + 1);
// console.log(z);

// const randomDicesRiolls = Array.from({length: 100}, () => Math.floor(Math.random() * 6) + 1);
// console.log(randomDicesRiolls);


// Important Note about array Methods ;
// Which array Method to use? I want : 

// 1. To mutate original array 
      // Muted Array Method Which Change the original Array 
            
            // Add to original : push() and unshift();
            // Remove from original : pop() , shift() and splice;
            // Others : reverse() , sort() and fill();


// 2. A new Array 
         // Computed from original : map() => loop
         // Filtering using condition : filter();
         // portion of original : slice();
         // Adding original to other : concat();
         // Flattening the original : flat() and flatMap();

// 3. An array index
         // findIndex() most usable;
         // indexOf();
...
const eathingOkay = dogs.filter(dog => dog.curFood > dog.recFood * 0.9 && dog.curFood < dog.recFood * 1);

console.log(eathingOkay)

const shallowCopyForSort = dogs.slice().sort((a, b) => b.recFood - a.recFood)
console.log(shallowCopyForSort)



// Array Methods more practice;
// 1.
...
// const numberDeopsit1000 = accounts.flatMap(acc => acc.movements).filter(mov => mov >= 1000);

// const numberDeopsit1000 = accounts.flatMap(acc => acc.movements)
                                 //  .reduce((count, curr) =>( curr >=  1000 ? count++ : count), 0)


// 3.

const {deposit, withdrawals}= accounts.flatMap(acc => acc.movements).reduce((sums, curr) => {
